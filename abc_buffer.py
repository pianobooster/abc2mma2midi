...
# the following chars need escaping [ ] - and ^ only if first
...
BAR_LINE = re.compile('[|:\\[\\]]')
IGNORE_ABC_START = re.compile('[- \n\t><]') # todo remove the '-' '<' '>'
NOT_IMPLEMENTED_IGNORE = re.compile('[-]')
NOT_IMPLEMENTED_ERROR_START = re.compile('[-]')
...
class AbcBuffer:
    ...
    def is_pitch_rest(self):
        return self.match(PITCH_OR_REST_START)
    ...
    def is_ignore_abc(self):
        return self.match(IGNORE_ABC_START)

    def skip_ignore_abc(self):
        while self.match(IGNORE_ABC_START):
            self.pop_char()
